...
class TimerecordingV2 extends React.Component {
  state = {
    ...
    base64:false,
    btnDisabled:false,
    logo: {uri:'https://'+this.props.route.params.props.constantsData.instance+'.quicksteps.ch/img/'+this.props.route.params.props.constantsData.instance+'.logo.png'}
  };
  ...
  checkContactPassword = async() => {
    ...
    await checkEmployerPassword(this.state.password).then(async res => {
      if(res) {
        if (res.status === 200) {
          let loggedin = res.employer_status === 1 || res.employer_status === 'reason_24' ;
          let loggedout = res.employer_status === 2 || res.employer_status === 0 || res.employer_status === 'reason_21';
          ...
          switch (res.employer_status){
            ...
            case(3): inpause = true ;break;
            case('reason_1'): loggedout = true ;break;
            case('reason_2'): loggedout = true ;break;
            case('reason_3'): loggedout = true ;break;
            case('reason_4'): loggedout = true ;break;
            case('reason_5'): loggedout = true ;break;
            case('reason_6'): loggedout = true ;break;
            case('reason_7'): loggedout = true ;break;
            case('reason_8'): loggedout = true ;break;
            case('reason_9'): loggedout = true ;break;
            case('reason_10'): loggedout = true ;break;
            case('reason_11'): loggedout = true ;break;
            case('reason_12'): loggedout = true ;break;
            case('reason_13'): loggedout = true ;break;
            case('reason_14'): loggedout = true ;break;
            case('reason_15'): loggedout = true ;break;
            case('reason_16'): loggedout = true ;break;
            case('reason_17'): loggedout = true ;break;
            case('reason_18'): loggedout = true ;break;
            case('reason_19'): loggedout = true ;break;
            case('reason_20'): loggedout = true ;break;
            case('reason_21'): loggedout = true ;break;
            case('reason_22'): loggedout = true ;break;
            case('reason_23'): loggedout = true ;break;
            case('reason_24'): loggedin = true ;break;
            case('reason_25'): loggedout = true ;break;
            default:
              loggedin = false ; loggedout = false ; inpause = false ;break;
          }
    ...
    await setEmployerStatus({
      ...
      if(res){
        if(res.status == 200){
          ...
          if(this.state.actionStatus === 1){ titleSuccessAlert = 'Erfolgreich eingeloggt' }
          if(this.state.actionStatus === 2){ titleSuccessAlert = 'Erfolgreich ausgeloggt' }
          if(this.state.actionStatus === 3){ titleSuccessAlert = 'Erfolgreich'}
  ...
  snap = async () => {
    if (this.camera) {
      ...
      if(picture.base64)
        this.state.base64 = picture.base64;
  ...
  loginBtn = () => {
    return (
        <Button disabled={this.state.btnDisabled} color='success' style={{width:'100%',height:width < 800 ? 50 :100}} onPress={async ()=>{
  ...
  logoutBtn = () => {
    return (
        <Button disabled={this.state.btnDisabled} color='error' style={{width:'100%',height:width < 800 ? 50 :100}} onPress={async ()=>{
  ...
  pauseStartBtn = () => {
    return (
        <Button disabled={this.state.btnDisabled} color={'default'} style={{width:'100%',height:width < 800 ? 50 :100}} onPress={async ()=>{
  ...
  infoBtn = () => {
    return (
        <TouchableOpacity style={{marginLeft:10,width:'100%'}} onPress={async ()=>{
          this.setState({spinner:true,actionsScreen:false})
          await getEmployerMonthData(this.state.contact_id).then((res)=> {
  ...
  infoScreen = () => {
    ...
    yearly_data.map((row,index) => {
      if(row.id === this.state.contact_id){
        ...
        remaining_holiday = remaining_holiday+(remaining_holiday === 1 ? ' Tag':' Tage');
      }
    });

    return (
        <Block center style={{width:'100%'}}>
          <Block  style={{flex:1,alignSelf: 'stretch', flexDirection: 'row',width:'100%',marginBottom: 20}}>
            <Block  style={{flex: 1}}>
              <Text style={{color:'#fff',fontSize: 16,fontWeight: 'bold'}}>Arbeitsstunden</Text>
              <Text style={{color:'#fff',fontSize: 16}}>{this.state.infoData.data2.hoursTotalTime}</Text>
            </Block>
            {this.state.infoData.data2.hoursTodoTime !== '00:00' ? (
                <Block  style={{flex: 1, }}>
                  <Text style={{color:'#fff',fontSize: 16,fontWeight: 'bold'}}>Sollstunden</Text>
                  <Text style={{color:'#fff',fontSize: 16}}>{this.state.infoData.data2.hoursTodoTime}</Text>
                </Block>
            ):false}
            <Block  style={{flex: 1}}>
              <Text style={{color:'#fff',fontSize: 16,fontWeight: 'bold'}}>Resturlaub</Text>
              <Text style={{color:'#fff',fontSize: 16}}>{remaining_holiday}</Text>
            </Block>
          </Block>
          <Table rows={this.state.infoData.data.dataSet.data}/>
          <Block center style={{width:'100%',marginTop:10}}>
            {this.state.backBtn ? this.backBtn({
              actionsScreen:true,
              infoScreen:false,
              showLogo:true,
              infoBtn:true,
  ...
  backBtn = (states) => {
    ...
    return (
        <Button  style={{width:'100%',height:width < 800 ? 50 :100,backgroundColor:'#353b4a',borderRadius:0}} onPress={()=>{
          Keyboard.dismiss()
  ...
  versionInfo = () => {
    return (
        <TouchableOpacity style={{marginLeft:1,width:'100%'}} onPress={()=>{
  ...
  getNewLocation = async () => {
    if(!this.location_permission)
      this.location_permission = await Location.requestForegroundPermissionsAsync();

    if(this.location_permission.status === 'granted') {
      try{
        await Location.watchPositionAsync({accuracy: Location.Accuracy.Highest, distanceInterval:10},(location) => {
  ...
  Logo = () => {
    return this.state.showLogo ? (
              <Block center style={{width:'70%',height:width < 700 ? '18%' :'25%',marginBottom: width < 700 ? 10 :20,marginTop:width < 700 ? 10 :20}}>
                <Image  onError={()=>this.setState({logo:Images.QSLogoWhite})} source={this.state.logo} style={{resizeMode:'contain',height:'100%',width:'100%'}} />
              </Block>
    ):false
  }
  ...
  UserImage = () => {
    ...
    return !this.state.showLogo&&this.state.actionsScreen ? (
              <Block center style={{width:'70%',height:width < 700 ? '18%' :'25%',marginBottom: width < 700 ? 10 :20,marginTop:width < 700 ? 10 :20}}>
                <Image  source={{uri:this.image_uri}} style={{resizeMode:'contain',height:'100%',width:'100%'}} />
              </Block>
    ):false
  }
  ...
  render() {
    return (
        ...
        >
          ...
          {this.UserImage()}
          <Block>
            {this.state.welcomeText ? this.welcomeText():false}
          </Block>
          <Block flex center >
            <StatusBar hidden={Platform.OS == 'ios' ? false:true}  translucent={true}/>
            <Block middle center style={{width:(width<800?width*0.9:width*0.8),borderRadius: 5}}>
          ...
          <AwesomeAlert
              ...
              customView={
                <Input
                    ...
                    autoFocus={true}
                    keyboardAppearance={'dark'}
                    onChangeText={(note)=>{this.state.note=note;}}
                    onSubmitEditing={()=>{}}
                />}
              closeOnTouchOutside={false}
              closeOnHardwareBackPress={false}
              ...
              actionContainerStyle={{itemAlign:'center'}}
              cancelButtonStyle={{width:100}}
              cancelButtonTextStyle={{textAlign:'center',fontSize: 15,color:'#585e61'}}
